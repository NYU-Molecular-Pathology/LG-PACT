...
get_probe_matrix <- function(run_hs_files) {
    all_rows <- lapply(run_hs_files, function(f) {
        sampleID  <- str_remove(basename(f), "_PerTargetCoverage\\.txt$")
...
get_sample_pairs <- function(SAMPLES_CSV) {
    ...
    #all_pairs$Normal <- str_split_fixed(all_pairs$Normal, "_", 2)[, 6]
