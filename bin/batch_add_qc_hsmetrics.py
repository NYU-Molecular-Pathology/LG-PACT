    ...
    try:
        with open(filepath, "r") as f:
            ...
            for row in reader:
                ...
                print(f"Pairing: {base_id} -> Tumor: {tumor_id}, Normal: {normal_id}")
...
def generate_flagstat_headers(tumor_id, normal_id, qc_data):
    ...
    return generate_metric_headers(
        "TUMOR", tumor_id, qc_data
...
def find_hsmetrics_sample(base_id, hsmetrics_data, has_block):
    ...
    for sample_id, metrics in hsmetrics_data.items():
        print(
            f"sample_id: {sample_id} base_id: {base_id} with has_block={has_block}"
        )
...
def generate_hsmetrics_headers(tumor_id, normal_id, hsmetrics_data):
    ...
    print(f"!!!!!tumor_base: {tumor_base}, normal_base: {normal_base}")
    print(f"!!!hsmetrics_data keys: {list(hsmetrics_data.keys())}")

    tumor_metrics = find_hsmetrics_sample(tumor_base, hsmetrics_data, has_block=True)

    if tumor_metrics is None:
        tumor_metrics = find_hsmetrics_sample(
            tumor_base, hsmetrics_data, has_block=False
        )

    print(f"tumor_metrics: {tumor_metrics}")
    normal_metrics = find_hsmetrics_sample(normal_base, hsmetrics_data, has_block=False)

    if normal_metrics is None:
        normal_metrics = find_hsmetrics_sample(
            normal_base, hsmetrics_data, has_block=True
        )

    print(f"normal_metrics: {normal_metrics}")

    return generate_hsmetrics_sample_headers(
        "TUMOR", tumor_metrics
...
def process_vcf_files(vcf_files, headers, args):
    ...
    for vcf_path in vcf_files:
        ...
        if args.dry_run:
            print(
                f"[DRY RUN] {vcf_basename} -> {os.path.basename(output_path)} ({len(headers)} headers)"
            )
        else:
            try:
                final_path = add_headers_to_vcf(
                    vcf_path, headers, output_path, args.insert_after
                )
...
def main():
    ...
    parser.add_argument(
        "-m", "--hsmetrics-file", required=True, help="Path to HSMetrics HTML file"
    )
    parser.add_argument(
        "-p", "--pairing-file", required=True, help="Path to tumor-normal pairing CSV"
    )
    parser.add_argument(
        "-d",
        "--vcf-dir",
        required=True,
        help="Directory containing VCF files (.vcf or .vcf.gz)",
    )
    parser.add_argument(
        "-o", "--output-dir", help="Output directory for processed VCFs"
    )
    parser.add_argument(
        "--pattern",
        default="*.vcf*",
    ...
    parser.add_argument(
        "--insert-after",
        choices=["fileformat", "msi_tmb", "contig"],
        default="msi_tmb",
        help="Where to insert headers (default: msi_tmb)",
    )
    parser.add_argument(
    ...
    for base_id, pair_info in pairs.items():
        ...
        print(f"pair_info['tumor']: {pair_info['tumor']}")
        ...
        print(f"\nProcessing ${hsmetrics_headers[0].split('.')[1]} pair: {base_id}")
